...
- Two tuning surfaces exist: **user settings** (popup UI, persisted in the `settings` storage key, normalized/clamped in `normalizeSettings` background.js:1276-1301) and **code constants** (edit the source, reload the extension).
...
| GIF capture FPS | `hotkeyBurstFps` = 5 | background.js:38, 1269-1275 | Only 5/10/15 accepted (`normalizeHotkeyBurstFps`); anything else snaps to 5. Effective FPS is further capped by stability mode and pressure tiers. |
| Report retention | `reportRetention` = 3 (clamp 1–10) | background.js:1315-1316, 2376-2396 | Popup Privacy & Stability panel. `saveReportSnapshotDetached` slices to this bound; imports honor the same value (report.js:12627) with a visible notice when older reports are trimmed. Raising grows storage payload and spool footprint linearly. |
...
| `hotkeyBurstJpegQuality` | 38 (clamp 20–95) | background.js:41 | Halved from 75 to keep exported bundles small (a burst-heavy report at q75 easily exceeds 400 MB). Stability mode still hard-caps at 75 (background.js:1842), so raising above 75 has no effect while stability is on. >85 triggers backpressure sooner. |
...
| Stop-drain timeout | 15000 ms (`STOP_FINALIZATION_DRAIN_TIMEOUT_MS`) | background.js:178, 2530-2537 | Stop finalization waits this long for the spool to go idle before snapshotting (`drainOutcome` drained/timeout). Raise for very large write backlogs on slow disks. |
...
| Queue depth caps (capture/process/write) | 6 / 12 / 18 | background.js:217-219 | Hitting any = severe. Raise `writeQueueMax` first. Hard enqueue overflow at 4× `captureQueueMax` (frame_spool.js:1033). |
...
| Decode worker | disabled (`decodeWorkerEnabled:false`), 1 worker, batch 1 | background.js:223-225 | Enable only if inline decode is a measured bottleneck; 3 consecutive worker errors (`DECODE_WORKER_ERROR_THRESHOLD`) permanently fall back inline (frame_spool.js:20). |
...
| Report retention | see §1 | background.js:1315-1316, 2376-2396 · report.js:12627 | Moved to a user setting (`reportRetention`, default 3, clamp 1–10). Enforced in `saveReportSnapshotDetached` and at import time with a visible notice when older reports are trimmed. |
...
- **Report opens right after stop and misses tail burst frames:** stop finalization now drains the spool for up to 15 s before snapshotting (background.js:2530-2537), so this should be rare; if it still occurs (drain timed out — check `drainOutcome`), reopen the report (refs retry) or raise the `waitForFrame` timeout (frame_spool.js:1301).
