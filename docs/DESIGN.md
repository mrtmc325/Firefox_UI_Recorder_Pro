...
Updated 2026-07-16: Tier-2 follow-up — batches 2B / 2D shipped on top of v1.19.0. No architectural rewrites; incremental behavior and one new storage surface.
- 2D.2 `render()` / `updateAux()` split — the report editor's `render()` performs the visible-viewport DOM pass, then defers auxiliary rebuilds (burst players, narration controllers, embed panels) via `requestIdleCallback(run, { timeout: 200 })` with a `setTimeout(run, 200)` fallback. Move/drag/rename step ops call `render()` only.
- 2B.4 coalesced `saveReports` — at most one storage write per 500 ms trailing flush, with guaranteed flush on `beforeunload` and `visibilitychange` → hidden. Preserves vault-locked semantics from v1.19.0 (locked vault still hard-refuses the write; awaited coalesced promise rejects at flush time instead of silently downgrading).
- 2B.7 step tags — new `ev.tags: string[]` field (backward-compatible; missing == []) with per-step chip input + saved filter chips. Data attributes bake into exported HTML so filters survive re-open.
- 2B.8 report + section-shell templates — new **Storage boundary** entry: `browser.storage.local` key `__uiRecorderReportTemplates` (cap `REPORT_TEMPLATES_MAX` = 20). A template captures theme + exportTheme + ordered section shells (title + subsection description). No events, no screenshots, no audio, no section text/media, no brand assets, no tag data. Titles for events without user-authored `editedTitle` are placeholder-filled to avoid recorded values (`ev.human`/`ev.label`/`ev.value`/`ev.text`) leaking into the template.
- 2D.3 lazy lowercased haystack cache — per-event lowercased search haystack lives in a per-page `WeakMap`, invalidated at every field mutation site (section description, step title/text, tags).
- 2B.9 Markdown / plain-text runbook export — clean Markdown (headings for sections, numbered lists for steps, code fences for URLs) with two modes: inline data-URI images, or sibling screenshot files packaged inside a small ZIP.
- 2B.11 Playwright test-scaffold emitter — `test.js` scaffold with best-effort selectors (aria-label / data-testid / role+name preferred over CSS class), a "SCAFFOLD — REVIEW BEFORE RUNNING" banner, sensitive-query-parameter URL scrubbing (`token`/`api_key`/`secret`/`auth`/`sig`/`session`/`jwt`/`access_token`/`bearer`/`code`), and placeholder replacement of free-text `page.fill()` values (checkbox / radio / select are preserved).
- 2B.1 recording-presets dropdown in the popup — one-click bundles for Default / SPA / Sensitive / Long-session (`SETTINGS_PRESETS` in `popup.js`). Non-destructive; individual settings remain editable after apply.
- 2B.5 storage-quota preflight + backpressure — `navigator.storage.estimate()` at recording start and every 30 s during recording. New `quota` backpressure tier pauses burst capture at `STORAGE_QUOTA_PAUSE_RATIO` = 0.85 (pause reason `storage-quota`); recording auto-stops at `STORAGE_QUOTA_STOP_RATIO` = 0.97. Poll cadence `STORAGE_QUOTA_POLL_INTERVAL_MS` = 30000.
- 2B.10 raw-bundle contract — `docs/BUNDLE_SCHEMA.md` (prose) + `docs/bundle-schema.json` (JSON Schema) document the raw ZIP bundle format v4 already produced/consumed by `report.js` for downstream tooling to validate exports.
- 2B.12 / 2B.13 / 2B.15 optest coverage — extended to redaction / `mergeReports` / ZIP parse; a v1.13.5-shape bundle round-trip fixture; and a diagnostics ring integration test (against the v1.19.0 `DIAGNOSTICS_RING_MAX` = 500 ring).
- 2D.1 vector annotations — design doc only (`docs/plans/vector-annotations-2026-07-16.md`); report editor exposes a button that opens the plan file. No drawing code yet, no persistent state.
- 2B.14 developer tooling — `docs/dev-run.sh` (`npx web-ext run` wrapper) and `docs/eslintrc.json` (eslint:recommended shape, on-demand via `npx eslint`). No persistent package.json / node_modules.

**Storage boundary refresh.** `browser.storage.local` gains a single new key: `__uiRecorderReportTemplates` (bounded by `REPORT_TEMPLATES_MAX` = 20). Template contents are theme/shell only — the writer explicitly excludes all recorded event data. Vault-mode does not apply to this key (templates carry no user event data).

**Risks & Mitigations addition.**

| Risk | Impact | Likelihood | Mitigation |
|---|---|---|---|
| Coalesced editor saves + tab close may lose the trailing flush for vault-encrypted payloads | Medium | Low | `visibilitychange` → hidden is preferred over `beforeunload` (fires earlier and reliably on tab-hide across browsers); the immediate-then-flush path is structured to minimize `await`s so the encryption promise has the best chance to resolve before teardown. Trade-off is documented in code near `_saveReportsFlushNow`. Users with active vault-mode editing should still explicitly Save before closing the tab. |

No open questions to add — both v1.19.0 open questions closed prior to this follow-up.

Updated 2026-07-17: Tier-2 deferred — batches 2B.2 / 2B.3 / 2B.6 shipped on top of v1.20.0. No architectural rewrites; incremental behavior on the report editor and popup, plus one new sessionStorage surface.
- 2B.2 rename/delete UI surface — the report editor's report-select row exposes `#report-rename` and `#report-delete` buttons. Rename prompts, runs `sanitizeReportTitle` (`/[^A-Za-z0-9 _.-]+/g` strip, 80-char cap; report.js), updates both the dropdown label and the report H1 in sync, and rides the coalesced `saveReports` write path. Delete confirms, removes the report by id, forces an immediate coalesced-save flush (bypasses the 500 ms window), and preserves the report if the write rejects. Both buttons disable during the save window to defeat rapid-double-click racing.
- 2B.3 dual undo rings — a per-report **in-memory** ring `stepUndoRing` (cap `STEP_UNDO_MAX` = 10) snapshots the events array before every move/drag/delete step operation; the "Undo last change" bar restores the prior array. screenshotRef ids are captured by string identity so vault-encrypted binary payloads are never duplicated into the snapshot. Separately, when Delete Report succeeds, a **sessionStorage** report-level undo snapshot is written under `firefox-ui-recorder-report-undo` with a 5-minute TTL so the post-reload page can offer Undo delete. The sessionStorage snapshot is gated on encrypted vault OFF AND secure-at-rest OFF — vault protection is never bypassed by the undo cache.
- 2B.6 cross-report search from popup — a search input above the settings card runs `searchReports(reports, query, CROSS_REPORT_SEARCH_CAP=20)` (popup.js, pure function) over already-loaded reports and renders a `role=listbox` result panel (arrow-key navigable, `aria-activedescendant`, Escape returns to input). Selecting a result posts `OPEN_REPORT` with `idx` and `stepId` params; background opens `report.html?idx=N#step-K` (deep-link fragment always the position-based `step-N` because report.js reassigns stepIds on render via `assignStepIds`). Report page scrolls the matching step into view (falling back to the enclosing burst wrap when the step was folded into a click burst). Text-only search over data already in memory — no new storage key, no new permissions, honors secure-at-rest.

**Storage boundary refresh.** No `browser.storage.local` change. One new **sessionStorage** key at report tab scope:
- `firefox-ui-recorder-report-undo` — per-tab, 5-minute TTL (`Date.now() - snap.ts > 5 * 60_000`), cleared on undo-consume and on expiry read. Explicitly out-of-scope for `browser.storage.local` (per-tab lifetime matches the delete-then-reload navigation without a delete-flow refactor). Gated on vault-off AND secure-at-rest-off; never written while vault or secure-at-rest is engaged.

**Runtime message extension.** `OPEN_REPORT` (background.js) accepts two new fields: `idx: number` (clamped to `>= 0`) and `stepId: string` (validated `/^[A-Za-z][A-Za-z0-9_-]{0,63}$/` — accepts `step-N` / `tpl_step_*` / `burst_*` and rejects fragment/URL-injection shapes). Legacy no-arg callers still open the report tab unchanged.
